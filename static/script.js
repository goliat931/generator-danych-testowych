...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("static/plewibnra_utf8.txt")
    ...
    .then((plewibnraContent) => {
      ...
      function getBankAndBranchCode(selectedBankCode, validCodes) {
        if (selectedBankCode === "random") {
          ...
          const filteredCodes = validCodes.filter((code) =>
          ...
          return filteredCodes[
            Math.floor(Math.random() * filteredCodes.length)
          ];
        }
      }
      ...
      function formatNrb(nrb, format, prefix) {
        let formattedNrb = nrb;
        if (format === "spaced") {
          formattedNrb = `${formattedNrb.substring(0, 2)} ${formattedNrb.substring(2, 6)} ${formattedNrb.substring(6, 10)} ${formattedNrb.substring(10, 14)} ${formattedNrb.substring(14, 18)} ${formattedNrb.substring(18, 22)} ${formattedNrb.substring(22, 26)}`;
        }

        if (prefix === "with-prefix") {
          if (format === "spaced") {
            formattedNrb = `PL ${formattedNrb}`;
          } else {
            formattedNrb = `PL${formattedNrb}`;
          }
        }
      ...
      function generateNrb(selectedBankCode, format, prefix) {
        const bankAndBranchCode = getBankAndBranchCode(
          selectedBankCode,
          validNrbCodes,
        );
        if (bankAndBranchCode.startsWith("Brak")) {
          return bankAndBranchCode; // Zwraca komunikat o błędzie
        }

        const customerNumber = generateCustomerNumber(16);
        ...
        const finalNrb = `${checksum}${fullNumberWithoutChecksum}`;

        return formatNrb(finalNrb, format, prefix);
      }
