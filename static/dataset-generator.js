document.addEventListener("DOMContentLoaded", () => {
  // ====================================================
  // 1. Inicjalizacja trybu ciemnego / Dark mode initialization
  // ====================================================
  function initTheme() {
  ...
  // ====================================================
  // 4. Funkcje generujące / Generator functions
  // ====================================================
  ...
  function getRandomSurname() {
    ...
    // Fallback na Faker jeśli nazwiska JSON się nie załadowały / Fallback to Faker if JSON surnames did not load
    try {
  ...
  function getRandomMaleName() {
    ...
    // Fallback na Faker jeśli nazwy JSON się nie załadowały / Fallback to Faker if JSON names did not load
    try {
  ...
  function generatePeselFromDate(date, sex) {
    ...
    // Zakoduj miesiąc w zależności od stulecia / Encode month depending on the century
  ...
  function getRandomCity() {
    // Kombinuj z polskimi miastami dla większej realistyczności / Combine with Polish cities for greater realism
    if (Math.random() > 0.4) {
      return polishCities[Math.floor(Math.random() * polishCities.length)];
    }
    // Fallback na polskie miasta jeśli Faker nie będzie dostępny / Fallback to Polish cities if Faker is unavailable
    try {
  ...
  function getRandomStreetName() {
    // Kombinuj z polskimi ulicami dla realistyczności / Combine with Polish streets for realism
    if (Math.random() > 0.4) {
      const name = streetNames[Math.floor(Math.random() * streetNames.length)];
      const prefix = Math.random() > 0.5 ? "ul." : "";
      return `${prefix} ${name}`.trim();
    }
    // Fallback na polskie ulice jeśli Faker nie będzie dostępny / Fallback to Polish streets if Faker is unavailable
    try {
  ...
  function generatePhoneNumber() {
    // Generuj numer telefoniczny w formacie polskim / Generate phone number in Polish format
  ...
  function generateEmail(first, last) {
    // Użyj Faker do generowania emaila, ale z polskimi domenami / Use Faker to generate email, but with Polish domains
  ...
  function generateCompanyName() {
    // Generuj nazwę firmy z polskiego słownika + sufiksy / Generate company name from Polish dictionary + suffixes
  ...
  // ====================================================
  // 5. Drag & Drop (zmiana kolejności) / Drag & Drop (reordering)
  // ====================================================
  ...
  document.querySelectorAll(".draggable-item").forEach((item) => {
    ...
    item.addEventListener("dragend", function () {
      ...
      // Usuń visual feedback ze wszystkich elementów / Remove visual feedback from all elements
  ...
  // ====================================================
  // 6. Generowanie i export (oddzielnie) / Generation and export (separately)
  // ====================================================
  ...
  document
    ...
    .addEventListener("click", () => {
      ...
      // Zamień "tab" na faktyczną tabulację / Replace 'tab' with actual tabulation
      if (separator === "tab") {
        separator = "\t";
      }

      // Pobierz zaznaczone pola w kolejności / Get selected fields in order
      const fields = [];
      document
      ...
      // Generuj dane / Generate data
      const data = [];
      for (let i = 0; i < recordCount; i++) {
        const record = {};

        // Podstawowe wartości używane w wielu polach / Basic values used in many fields
        const sex = "M";
      ...
      // Przygotuj export / Prepare export
      ...
      // Przechowaj dane do pobrania / Store data for download
      generatedData = data;
      ...
      // Aktywuj przycisk pobierania / Activate download button
      document.getElementById("downloadDatasetBtn").disabled = false;
    });

  // Przycisk pobierania / Download button
  document
  ...
  // ====================================================
  // 7. Generowanie CSV / CSV generation
  // ====================================================
    ...
    const rows = data.map((record) =>
      fields
        .map((field) => {
          ...
          // Escape wartości zawierające separator, cudzysłowy lub nowe linie / Escape values containing separator, quotes or newlines
          if (
            typeof value === "string" &&
  ...
  // ====================================================
  // 8. Generowanie XML / XML generation
  // ====================================================
  ...
  // ====================================================
  // 10. Pobieranie pliku / File download
  // ====================================================
