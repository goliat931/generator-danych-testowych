    ...
    .addEventListener("click", () => {
      ...
      document
        ...
        .forEach((checkbox) => {
          ...
          let customName = nameInput ? nameInput.value.trim() : checkbox.value;
          customName = customName || checkbox.value;

          // Ensure uniqueness
          if (nameCounts[customName] !== undefined) {
            nameCounts[customName]++;
            customName = `${customName}_${nameCounts[customName]}`;
          } else {
            nameCounts[customName] = 0;
          }

          fieldsInfo.push({ key: checkbox.value, name: customName });
        });
