...
## 2025-02-23 - Focus Visible Outline on Inputs
**Learning:** Setting `outline: none` relying only on `border-color` changes on inputs/selects on focus is an anti-pattern. This strips out the focus indicators completely in High Contrast Modes, making it difficult for keyboard users to navigate.
**Action:** Always use `outline: 2px solid transparent;` on `:focus` to preserve native high-contrast borders while applying custom styles via `:focus-visible`. Additionally, explicitly add `input`, `select`, and `textarea` to global `:focus-visible` styles to ensure clear keyboard focus indicators.
