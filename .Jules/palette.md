...
## 2025-02-23 - Inline Validation Feedback and Accessible Error States
**Learning:** Text inputs inside the validator page did not programmatically relate to their error message containers, meaning screen reader users wouldn't automatically hear the error text associated with the input. Furthermore, there was no visual indication on the input itself when it contained an error, only the text below it.
**Action:** Always link form inputs to their error containers using `aria-describedby="containerId"`. Use JavaScript to dynamically set `aria-invalid="true"` or `"false"` upon validation, and use the `input[aria-invalid="true"]` CSS selector to tie visual cues (like a red border) directly to the accessible state. Finally, clear the validation feedback (`aria-invalid` and text) immediately on the `input` event to give real-time feedback as the user corrects their mistake.
