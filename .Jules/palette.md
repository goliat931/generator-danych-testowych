...
## 2024-07-25 - Form Validation Accessibility & Semantic Cues
**Learning:** Tying visual validation states directly to the semantic `aria-invalid` attribute, along with dynamically associating error message containers using `aria-describedby`, ensures screen reader users have immediate context. Furthermore, clearing the `aria-invalid` state precisely on the `input` event immediately reassures users and reduces cognitive load by removing errors as soon as they start correcting them.
**Action:** When implementing or refactoring custom form validation, always link input errors to semantic roles (`aria-describedby` and `aria-invalid`). Avoid generic class-based visual styling (`.error-border`) in favor of styling based directly on `[aria-invalid="true"]`. Always clear these states on the `input` event to provide real-time feedback.
