...
## 2026-07-08 - Accessible Form Validation Feedback
**Learning:** Using generic classes for valid/invalid states (`.valid`, `.invalid`) decoupled from the actual semantic state of the input element means that styling cues can easily get out of sync with what screen readers perceive. Furthermore, visual indicators alone (like an error message below) aren't always enough to draw immediate attention to the specific field that needs correction.
**Action:** Always link form inputs to their error messages using `aria-describedby` and dynamically set `aria-invalid="true"` or `"false"` on the input itself when validation occurs. Then, use CSS attribute selectors (e.g., `input[aria-invalid="true"]`) to apply the visual error styles directly to the input (like a red border and slightly tinted background). Finally, ensure you clear the `aria-invalid` state immediately on the `input` event when the user starts correcting the value to provide responsive, real-time feedback.
