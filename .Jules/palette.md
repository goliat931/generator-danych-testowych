...
## 2025-02-18 - Preserve High Contrast Mode with Transparent Outlines
**Learning:** Using `outline: none;` on interactive elements like `input` and `select` removes the native focus indicator, which is a critical anti-pattern for High Contrast Mode (HCM) users, as custom border colors are often overridden by the OS.
**Action:** Always use `outline: 2px solid transparent;` instead of `outline: none;` on `:focus` to maintain the native HCM border, and apply custom focus rings using `:focus-visible` with `outline` and `outline-offset`.
